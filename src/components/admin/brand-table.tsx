...
export default function BrandTable({ brands }: { brands: BrandRow[] }) {
  ...
  const handleToggle = async (id: string, ativo: boolean) => {
    try {
      await updateBrand(id, { ativo });
      router.refresh();
    } catch (e: any) {
      setActionError(e.message || "Erro ao alterar marca");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setActionError(e.message || "Erro ao excluir marca");
    }
  };
  ...
  if (brands.length === 0) {
    return (
      <>
        <RealtimeRefresh tables={["brands"]} />
        <div className="rounded-xl border border-[#dfe7dd] bg-white p-12 text-center">
          <p className="text-[#8c9c91]">Nenhuma marca encontrada.</p>
        </div>
      </>
    );
  }
      ...
      <div className="overflow-x-auto rounded-xl border border-[#dfe7dd] bg-white shadow-sm">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#f0f4f0]">
            {brands.map((brand) => {
              const productCount = Array.isArray(brand.products) ? brand.products.length : (brand.products?.count ?? 0);
              return (
                <tr key={brand.id} className="hover:bg-[#fcfdfa]">
                  <td className="px-4 py-3 font-medium text-[#102016]">{brand.name}</td>
                  <td className="px-4 py-3 text-[#8c9c91] max-w-[200px] truncate">{brand.description || "-"}</td>
                  <td className="px-4 py-3 text-[#526157]">{productCount}</td>
                  <td className="px-4 py-3">
                    <Badge variant={brand.ativo ? "default" : "secondary"} className={brand.ativo ? "bg-green-100 text-green-700" : ""}>
                      {brand.ativo ? "Ativo" : "Inativo"}
                    </Badge>
                  </td>
                  <td className="px-4 py-3 text-[#8c9c91]">{formatDate(brand.created_at)}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex gap-1 justify-end">
                      <Link href={`/admin/marcas/${brand.id}`}>
                        <Button size="sm" variant="outline"><Pencil className="h-3 w-3" /></Button>
                      </Link>
                      <Button size="sm" variant="outline" className="text-red-600" onClick={() => setConfirmDelete({ id: brand.id, name: brand.name, productCount })}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setConfirmDelete(null)}>
          <div className="w-full max-w-md mx-4 rounded-xl border border-[#dfe7dd] bg-white p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-lg font-bold text-[#102016]">Excluir marca</h3>
            ...
            {confirmDelete.productCount > 0 && (
              <p className="text-sm text-amber-600 mb-6">
                {confirmDelete.productCount} produto(s) perderão a referência a esta marca.
              </p>
            )}
            {confirmDelete.productCount === 0 && (
              <p className="text-sm text-[#8c9c91] mb-6">Nenhum produto está vinculado a esta marca.</p>
            )}
