...
import { Plus, Search, CheckCircle2, Ban, Receipt } from "lucide-react"
import {
  listContasReceber,
  marcarComoRecebido,
  updateContaReceber,
  getResumoContasReceber,
} from "@/app/admin/financeiro/contas-receber-actions"
...
export function ContasReceberInline() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const result = await listContasReceber({ status, search })
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Filters + Actions */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#dfe7dd] bg-white p-4 shadow-sm">
        <div className="flex flex-wrap gap-3 items-end">
          <div>
            <label className="text-xs font-semibold text-[#526157] mb-1 block">Status</label>
            <select value={status} onChange={e => setStatus(e.target.value)} className="h-9 rounded-md border border-[#dfe7dd] px-3 text-sm">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Descrição..."
                className="h-9 rounded-md border border-[#dfe7dd] pl-8 pr-3 text-sm w-48" />
            </div>
          </div>
          <Button variant="outline" className="h-10" onClick={fetchData}>Filtrar</Button>
        </div>
        <div className="flex gap-2">
          <Link href="/admin/financeiro/contas-receber">
            <Button variant="outline" className="h-10 text-sm">
              <Receipt className="h-4 w-4 mr-2" />Ver Completo
            </Button>
          </Link>
          <Link href="/admin/financeiro/novo?type=RECEITA">
            <Button className="bg-[#006B2E] text-white hover:bg-[#005324] h-10">
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-[#f0f4f0]">
              {contas.map(conta => {
                ...
                return (
                  <tr key={conta.id} className="hover:bg-[#fcfdfa]">
                    <td className="px-4 py-3 font-medium text-[#102016] max-w-[160px] truncate">{conta.descricao || "-"}</td>
                      ...
                      {showDatePicker === conta.id && (
                        <div className="mt-2 flex items-center gap-2 p-2 rounded-lg border border-green-200 bg-green-50">
                          <input type="date" value={dataRecebimento} onChange={e => setDataRecebimento(e.target.value)}
                            className="h-8 rounded-md border border-[#dfe7dd] px-2 text-xs flex-1" defaultValue={today} />
                          <Button size="sm" className="h-8 bg-[#006B2E] text-white text-xs" onClick={() => handleMarcarRecebido(conta.id)}>Confirmar</Button>
                          <button onClick={() => setShowDatePicker(null)} className="text-xs text-[#526157] hover:underline">Cancelar</button>
                        </div>
                      )}
                    </td>
                  </tr>
              ...
              {contas.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-10 text-center text-[#8c9c91]">Nenhuma conta a receber encontrada.</td></tr>
              )}
            </tbody>
          </table>
        )}
